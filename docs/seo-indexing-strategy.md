...
- 실제 입력 문자열: `ㅊ미`, `yoi`, `chatgtp`, `gamil`, `drvie`
- Calendar 대표 변형: `ㅊ미`, `ccal`, `caal`, `call`
- 주소창 alias: `cal`, `youtube`, `chatgpt`, `claude`, `naver`, `github`
- 자판 입력 실수: `cal -> ㅊ미`, `youtube -> ㅛㅐㅕ셔ㅠㄷ`
...
## 2026-05-21 adjustment

- `오타`는 사용자가 직접 검색하는 핵심 쿼리라기보다 페이지 맥락을 설명하는 단어로 봅니다.
- 메타 keywords와 랜딩 검색어 후보에서는 `{서비스명} 오타` 조합을 제외합니다.
- `잘못침`, `한영`, `검색어`, `바로가기`처럼 사용자가 실제로 잘못 입력한 문자열이 아닌 진단형 표현도 타깃 쿼리에서 제외합니다.
- 헤딩, 설명문, 구조화 데이터에는 필요한 만큼만 남겨 Google과 사용자에게 페이지 의도를 설명합니다.
